...
*
*/
int addEmployee(Employee* list, int len, int id, char name[],char lastName[],float salary,int sector) //ESTO ESTA MAL ARREGLA
{
    ...
    if( list != NULL && len > 0 && id > 0)
    {
        ...
        else
        {
            ...
            nuevaPersona.name[51] = name;
            nuevaPersona.lastName[51] = lastName;
...
int removeEmployee(Employee* list, int len, int id)
{
    ...
    if(list != NULL && len > 0)
    {
        ...
        else
        {
            ...
            {
                error = 2;
                printf("la baja se dio por el usuario \n\n");
            }
        }
    }
return -1;
...
int sortEmployees(Employee* list, int len, int order)
{
    ...
    if(list != NULL && len > 0 && order >= 0 && order <= 1)
    {
        for(int i=0; i < len -1; i++)
        {
            ...
            {
                if((strcmp(list[i].name, list[j].name) > 0 && order && strcmp(list[i].sector, list[j].sector) > 0 ) || (strcmp(list[i].name, list[j].name) < 0 && !order && strcmp(list[i].sector, list[j].sector) > 0))
                {
                    auxPersona = list[i];
                    list[i] = list[j];
                    list[j] = auxPersona;
...
int modifyEmployee(Employee* list, int len)
{
    ...
    if(list != NULL && len > 0)
    {
        ...
        else
        {
            ...
            char newName = list[indice].name;
            char newLastName = list[indice].lastName;
            float newSalario = list[indice].salary;
            int newSector = list[indice].sector;
            ...
            if(confirma == 's')
            {
                nuevaPersona.name[51] = newName;
                nuevaPersona.lastName[51] = newLastName;
